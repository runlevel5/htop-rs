...
use super::Crt;
use crate::core::{
    highlight_flags, FieldWidths, Machine, Process, ProcessField, ProcessState, Settings,
};
#[cfg(target_os = "linux")]
...
impl MainPanel {
                ...
                if let Some(&idx) = pid_to_idx.get(&pid) {
                    if let Some(process) = machine.processes.processes.get(idx) {
                        ...
                        if process.is_visible
                            && self.should_show_process(process, settings, machine)
                        {
    ...
    fn draw_header(
        ...
        let command_filter_active = filter_active && self.fields.contains(&ProcessField::Command);
        ...
        for field in &self.fields {
            ...
            let text_attr = if is_sort_column { sort_attr } else { base_attr };
    ...
    fn draw_process(
        ...
        // Match C htop Panel_draw: use selection_color when focused, PANEL_SELECTION_UNFOCUS when not
        ...
        // Determine highlight attribute for new/tomb processes (if highlight_changes enabled)
        // This matches C htop Row_display logic
        let highlight_attr = if settings.highlight_changes {
            if process.is_tomb() {
                Some(crt.color(ColorElement::ProcessTomb))
            } else if process.is_new(
                monotonic_ms,
                settings.highlight_delay_secs,
                initial_scan_done,
            ) {
                Some(crt.color(ColorElement::ProcessNew))
            } else {
                None
            }
        } else {
    ...
    fn write_field(
        ...
        match field {
            ProcessField::Pid => {
                ...
                str.append(
                    &format!("{:>width$} ", process.pid, width = width),
                    base_color,
                );
            }
            ProcessField::Ppid => {
                ...
                let width = field_widths.pid_digits;
                str.append(
                    &format!("{:>width$} ", process.ppid, width = width),
                    base_color,
                );
            }
            ProcessField::User => {
            ...
            ProcessField::PercentCpu => {
                ...
                if is_shadowed {
                    str.append(
                        &format!("{:>width$.1} ", process.percent_cpu, width = width),
                        shadow_color,
                    );
                } else {
                ...
                if let Some(ref cmd_str) = merged_command.str_value {
                    ...
                    if highlight_base_name {
                        if process.cmdline.is_some() {
                            let basename_len = process
                                .cmdline_basename_end
                                .saturating_sub(process.cmdline_basename_start);
                            if basename_len > 0 {
            ...
            ProcessField::IORate => {
                ...
                let read_rate = if process.io_rate_read_bps.is_nan() {
                    0.0
                } else {
                    process.io_rate_read_bps
                };
                let write_rate = if process.io_rate_write_bps.is_nan() {
                    0.0
                } else {
            ...
            ProcessField::Pgrp => {
                ...
                let width = field_widths.pid_digits;
                str.append(
                    &format!("{:>width$} ", process.pgrp, width = width),
                    base_color,
                );
            }
            ProcessField::Session => {
                ...
                let width = field_widths.pid_digits;
                str.append(
                    &format!("{:>width$} ", process.session, width = width),
                    base_color,
                );
            }
            ProcessField::Tpgid => {
                ...
                let width = field_widths.pid_digits;
                str.append(
                    &format!("{:>width$} ", process.tpgid, width = width),
                    base_color,
                );
            }
            ProcessField::Minflt => {
            ...
            ProcessField::StUid => {
                ...
                str.append(
                    &format!("{:>width$} ", process.uid, width = width),
                    base_color,
                );
            }
            ProcessField::Tgid => {
            ...
            ProcessField::PercentNormCpu => {
                ...
                if is_shadowed {
                    str.append(
                        &format!("{:>width$.1} ", norm_cpu, width = width),
                        shadow_color,
                    );
                } else {
            ...
            ProcessField::Cnclwb => {
                ...
                if process.io_cancelled_write_bytes == u64::MAX {
                    ...
                    print_bytes(
                        str,
                        process.io_cancelled_write_bytes,
                        coloring && !is_shadowed,
                        crt,
                    );
                }
    ...
    /// Draw the panel
    /// `focus` indicates whether this panel currently has focus (for selection color)
    pub fn draw(&mut self, crt: &mut Crt, machine: &mut Machine, settings: &Settings, focus: bool) {
        let visible_height = if self.show_header { self.h - 1 } else { self.h };
        ...
        let monotonic_ms = machine.monotonic_ms;
        let initial_scan_done = machine.initial_scan_done;
    ...
    fn handle_search_key(&mut self, key: i32, machine: &Machine) -> HandlerResult {
        ...
        match key {
            ...
            KEY_BACKSPACE | KEY_DEL_MAC | KEY_CTRL_BS => {
            ...
            ch if (KEY_PRINTABLE_START..KEY_PRINTABLE_END).contains(&ch) => {
    ...
    pub fn try_select_pid(&mut self, pid: i32, machine: &Machine, settings: &Settings) -> bool {
        let processes: Vec<&Process> = if self.tree_view {
            machine
                .processes
                .iter_tree()
