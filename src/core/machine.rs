...
#[cfg(target_os = "linux")]
use super::process::ScanFlags;
use super::process::{CommandStrParams, Process, ProcessField, ProcessList};

/// Memory type alias (in KB, unsigned for values that are always present)
pub type Memory = u64;

/// Optional memory type alias (in KB, -1 means not available on this platform)
pub type OptionalMemory = i64;

/// Constant for "not available" optional memory value
pub const MEMORY_NOT_AVAILABLE: OptionalMemory = -1;
...
pub struct Machine {
    ...
    pub shared_mem: OptionalMemory, // -1 if not available (e.g., macOS)
    pub available_mem: OptionalMemory, // -1 if not available
    pub compressed_mem: OptionalMemory, // -1 if not available (only macOS has this)
    ...
    // File descriptors
    pub fd_used: Option<u64>, // Number of allocated file descriptors (None if unknown)
    pub fd_max: Option<u64>,  // Maximum file descriptors (None if unlimited/unknown)
    ...
    // Track if initial scan/draw cycle is complete (for highlight_changes feature)
...
impl Machine {
    pub fn new(filter_user_id: Option<u32>) -> Self {
        ...
        Machine {
            ...
            buffers_mem: 0,
            cached_mem: 0,
            shared_mem: MEMORY_NOT_AVAILABLE,
            available_mem: MEMORY_NOT_AVAILABLE,
            compressed_mem: MEMORY_NOT_AVAILABLE,
            total_swap: 0,
            used_swap: 0,
            cached_swap: 0,
            active_cpus: 1,
