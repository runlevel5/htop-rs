...
impl Settings {
    ...
    pub fn load(&mut self) -> anyhow::Result<()> {
        ...
        for line in reader.lines() {
            ...
            if let Some((key, value)) = line.split_once('=') {
                ...
                // Legacy meter format
                if key == "left_meters" {
                    legacy_left_meters = value.split_whitespace().map(|s| s.to_string()).collect();
                    continue;
                }
                if key == "right_meters" {
                    legacy_right_meters = value.split_whitespace().map(|s| s.to_string()).collect();
                    continue;
                }
    ...
    fn parse_screen_definition(&self, name: &str, fields_str: &str) -> ScreenSettings {
        let fields: Vec<ProcessField> = fields_str
            .split_whitespace()
            .filter_map(ProcessField::from_name)
            .collect();
    ...
    pub fn write(&self) -> anyhow::Result<()> {
        ...
        for (idx, column) in self.header_columns.iter().enumerate() {
            ...
            // Write meter modes
            let meter_modes: Vec<String> =
                column.iter().map(|m| m.mode.to_i32().to_string()).collect();
...
mod tests {
    ...
    fn test_meter_mode_from_i32_valid() {
        ...
        assert_eq!(MeterMode::from_i32(5), MeterMode::StackedGraph);
    }
    ...
    fn test_meter_mode_from_i32_invalid() {
        ...
        assert_eq!(MeterMode::from_i32(6), MeterMode::Bar);
        assert_eq!(MeterMode::from_i32(-1), MeterMode::Bar);
    ...
    fn test_meter_mode_to_i32() {
        ...
        assert_eq!(MeterMode::StackedGraph.to_i32(), 5);
    }
    ...
    fn test_parse_meter_name_various_meters() {
        ...
        assert_eq!(
            parse_meter_name("LoadAverage"),
            ("LoadAverage".to_string(), 0)
        );
