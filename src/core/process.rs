...
// Scan flags indicating which expensive /proc reads are needed.
// These flags match C htop's PROCESS_FLAG_* constants.
// Used to skip expensive reads when the corresponding columns aren't displayed.
#[cfg(target_os = "linux")]
bitflags::bitflags! {
    #[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
    pub struct ScanFlags: u32 {
        /// Read /proc/[pid]/io for IO statistics
        const IO           = 0x0000_0001;
        /// Read /proc/[pid]/cwd for current working directory
        const CWD          = 0x0000_0002;
        /// Get scheduler policy via sched_getscheduler
        const SCHED_POLICY = 0x0000_0004;
        /// Get IO priority via ioprio_get syscall
        const IO_PRIORITY  = 0x0000_0100;
        /// Read /proc/[pid]/cgroup for cgroup info
        const CGROUP       = 0x0000_0800;
        /// Read /proc/[pid]/oom_score for OOM score
        const OOM          = 0x0000_1000;
        /// Read /proc/[pid]/smaps_rollup for PSS/swap info
        const SMAPS        = 0x0000_2000;
        /// Read /proc/[pid]/status for context switches
        const CTXT         = 0x0000_4000;
        /// Read /proc/[pid]/attr/current for security attributes
        const SEC_ATTR     = 0x0000_8000;
        /// Read /proc/[pid]/maps for library size calculation
        const LRS_FIX      = 0x0001_0000;
        /// Read delay accounting via netlink
        const DELAY_ACCT   = 0x0004_0000;
        /// Read /proc/[pid]/autogroup for autogroup info
        const AUTOGROUP    = 0x0008_0000;
        /// Read GPU-specific data
...
#[cfg(target_os = "linux")]
impl ScanFlags {
    /// Compute scan flags from a list of process fields.
    /// This ORs together the flags required by each field.
    pub fn from_fields(fields: &[ProcessField]) -> Self {
...
impl ProcessField {
    ...
    /// Get the scan flags required for this process field.
    /// These flags indicate which expensive /proc reads are needed.
    #[cfg(target_os = "linux")]
    pub fn flags(self) -> ScanFlags {
        match self {
            // IO fields require reading /proc/[pid]/io
            ProcessField::Rchar
            | ProcessField::Wchar
            | ProcessField::Syscr
            | ProcessField::Syscw
            ...
            | ProcessField::IOReadRate
            | ProcessField::IOWriteRate
            | ProcessField::IORate => ScanFlags::IO,

            // CWD field requires reading /proc/[pid]/cwd
            ProcessField::Cwd => ScanFlags::CWD,

            // Scheduler policy requires sched_getscheduler syscall
            ProcessField::SchedulerPolicy => ScanFlags::SCHED_POLICY,

            // IO Priority requires ioprio_get syscall
            ProcessField::IOPriority => ScanFlags::IO_PRIORITY,

            // CGroup fields require reading /proc/[pid]/cgroup
            ProcessField::CGroup | ProcessField::CCGroup | ProcessField::Container => {
                ScanFlags::CGROUP
            }

            // OOM score requires reading /proc/[pid]/oom_score
            ProcessField::Oom => ScanFlags::OOM,

            // Delay accounting fields require netlink
            ProcessField::PercentCpuDelay
            | ProcessField::PercentIODelay
            | ProcessField::PercentSwapDelay => ScanFlags::DELAY_ACCT,

            // SMAPS fields require reading /proc/[pid]/smaps_rollup
            ProcessField::MPss | ProcessField::MSwap | ProcessField::MPsswp => ScanFlags::SMAPS,

            // Context switches require reading /proc/[pid]/status
            ProcessField::Ctxt => ScanFlags::CTXT,

            // Security attributes require reading /proc/[pid]/attr/current
            ProcessField::SecAttr => ScanFlags::SEC_ATTR,

            // Autogroup fields require reading /proc/[pid]/autogroup
            ProcessField::AutogroupId | ProcessField::AutogroupNice => ScanFlags::AUTOGROUP,

            // LRS (library size) requires reading /proc/[pid]/maps
            ProcessField::MLib => ScanFlags::LRS_FIX,

            // Container detection uses cgroup info
            ProcessField::IsContainer => ScanFlags::CONTAINER,
    ...
    /// Create a ProcessField from its numeric ID (matches C htop field IDs)
    pub fn from_id(id: u32) -> Option<ProcessField> {
        match id {
            // Common fields
            1 => Some(ProcessField::Command),
            2 => Some(ProcessField::State),
            3 => Some(ProcessField::Ppid),
            4 => Some(ProcessField::Pgrp),
            5 => Some(ProcessField::Session),
            6 => Some(ProcessField::Tty),
            7 => Some(ProcessField::Tpgid),
            9 => Some(ProcessField::Minflt),
            10 => Some(ProcessField::Majflt),
            18 => Some(ProcessField::Priority),
            19 => Some(ProcessField::Nice),
            21 => Some(ProcessField::Starttime),
            38 => Some(ProcessField::Processor),
            39 => Some(ProcessField::MSize),
            40 => Some(ProcessField::MResident),
            46 => Some(ProcessField::StUid),
            47 => Some(ProcessField::PercentCpu),
            48 => Some(ProcessField::PercentMem),
            49 => Some(ProcessField::User),
            50 => Some(ProcessField::Time),
            51 => Some(ProcessField::Nlwp),
            52 => Some(ProcessField::Tgid),
            53 => Some(ProcessField::PercentNormCpu),
            54 => Some(ProcessField::Elapsed),
            55 => Some(ProcessField::SchedulerPolicy),
            124 => Some(ProcessField::ProcComm),
            125 => Some(ProcessField::ProcExe),
            126 => Some(ProcessField::Cwd),

            // Linux-specific fields
            #[cfg(target_os = "linux")]
            11 => Some(ProcessField::Cminflt),
            #[cfg(target_os = "linux")]
            13 => Some(ProcessField::Cmajflt),
            #[cfg(target_os = "linux")]
            14 => Some(ProcessField::Utime),
            #[cfg(target_os = "linux")]
            15 => Some(ProcessField::Stime),
            #[cfg(target_os = "linux")]
            16 => Some(ProcessField::Cutime),
            #[cfg(target_os = "linux")]
            17 => Some(ProcessField::Cstime),
            #[cfg(target_os = "linux")]
            41 => Some(ProcessField::MShare),
            #[cfg(target_os = "linux")]
            42 => Some(ProcessField::MText),
            #[cfg(target_os = "linux")]
            43 => Some(ProcessField::MData),
            #[cfg(target_os = "linux")]
            44 => Some(ProcessField::MLib),
            #[cfg(target_os = "linux")]
            103 => Some(ProcessField::Rchar),
            #[cfg(target_os = "linux")]
            104 => Some(ProcessField::Wchar),
            #[cfg(target_os = "linux")]
            105 => Some(ProcessField::Syscr),
            #[cfg(target_os = "linux")]
            106 => Some(ProcessField::Syscw),
            #[cfg(target_os = "linux")]
            107 => Some(ProcessField::Rbytes),
            #[cfg(target_os = "linux")]
            108 => Some(ProcessField::Wbytes),
            #[cfg(target_os = "linux")]
            109 => Some(ProcessField::Cnclwb),
            #[cfg(target_os = "linux")]
            110 => Some(ProcessField::IOReadRate),
            #[cfg(target_os = "linux")]
            111 => Some(ProcessField::IOWriteRate),
            #[cfg(target_os = "linux")]
            112 => Some(ProcessField::IORate),
            #[cfg(target_os = "linux")]
            113 => Some(ProcessField::CGroup),
            #[cfg(target_os = "linux")]
            114 => Some(ProcessField::Oom),
            #[cfg(target_os = "linux")]
            115 => Some(ProcessField::IOPriority),
            #[cfg(target_os = "linux")]
            116 => Some(ProcessField::PercentCpuDelay),
            #[cfg(target_os = "linux")]
            117 => Some(ProcessField::PercentIODelay),
            #[cfg(target_os = "linux")]
            118 => Some(ProcessField::PercentSwapDelay),
            #[cfg(target_os = "linux")]
            119 => Some(ProcessField::MPss),
            #[cfg(target_os = "linux")]
            120 => Some(ProcessField::MSwap),
            #[cfg(target_os = "linux")]
            121 => Some(ProcessField::MPsswp),
            #[cfg(target_os = "linux")]
            122 => Some(ProcessField::Ctxt),
            #[cfg(target_os = "linux")]
            123 => Some(ProcessField::SecAttr),
            #[cfg(target_os = "linux")]
            127 => Some(ProcessField::AutogroupId),
            #[cfg(target_os = "linux")]
            128 => Some(ProcessField::AutogroupNice),
            #[cfg(target_os = "linux")]
            129 => Some(ProcessField::CCGroup),
            #[cfg(target_os = "linux")]
            130 => Some(ProcessField::Container),
            #[cfg(target_os = "linux")]
            131 => Some(ProcessField::MPriv),
            #[cfg(target_os = "linux")]
            132 => Some(ProcessField::GpuTime),
            #[cfg(target_os = "linux")]
            133 => Some(ProcessField::GpuPercent),
            #[cfg(target_os = "linux")]
            134 => Some(ProcessField::IsContainer),

            // macOS-specific fields
            #[cfg(target_os = "macos")]
            100 => Some(ProcessField::Translated),

            _ => None,
        }
    }

    /// Get the numeric ID of this field (matches C htop field IDs)
    pub fn to_id(self) -> u32 {
        match self {
            // Common fields
            ProcessField::Pid => 0, // 0 is special (terminator)
            ProcessField::Command => 1,
            ProcessField::State => 2,
            ProcessField::Ppid => 3,
            ProcessField::Pgrp => 4,
            ProcessField::Session => 5,
            ProcessField::Tty => 6,
            ProcessField::Tpgid => 7,
            ProcessField::Minflt => 9,
            ProcessField::Majflt => 10,
            ProcessField::Priority => 18,
            ProcessField::Nice => 19,
            ProcessField::Starttime => 21,
            ProcessField::Processor => 38,
            ProcessField::MSize => 39,
            ProcessField::MResident => 40,
            ProcessField::StUid => 46,
            ProcessField::PercentCpu => 47,
            ProcessField::PercentMem => 48,
            ProcessField::User => 49,
            ProcessField::Time => 50,
            ProcessField::Nlwp => 51,
            ProcessField::Tgid => 52,
            ProcessField::PercentNormCpu => 53,
            ProcessField::Elapsed => 54,
            ProcessField::SchedulerPolicy => 55,
            ProcessField::ProcComm => 124,
            ProcessField::ProcExe => 125,
            ProcessField::Cwd => 126,

            // Linux-specific fields
            #[cfg(target_os = "linux")]
            ProcessField::Cminflt => 11,
            #[cfg(target_os = "linux")]
            ProcessField::Cmajflt => 13,
            #[cfg(target_os = "linux")]
            ProcessField::Utime => 14,
            #[cfg(target_os = "linux")]
            ProcessField::Stime => 15,
            #[cfg(target_os = "linux")]
            ProcessField::Cutime => 16,
            #[cfg(target_os = "linux")]
            ProcessField::Cstime => 17,
            #[cfg(target_os = "linux")]
            ProcessField::MShare => 41,
            #[cfg(target_os = "linux")]
            ProcessField::MText => 42,
            #[cfg(target_os = "linux")]
            ProcessField::MData => 43,
            #[cfg(target_os = "linux")]
            ProcessField::MLib => 44,
            #[cfg(target_os = "linux")]
            ProcessField::Rchar => 103,
            #[cfg(target_os = "linux")]
            ProcessField::Wchar => 104,
            #[cfg(target_os = "linux")]
            ProcessField::Syscr => 105,
            #[cfg(target_os = "linux")]
            ProcessField::Syscw => 106,
            #[cfg(target_os = "linux")]
            ProcessField::Rbytes => 107,
            #[cfg(target_os = "linux")]
            ProcessField::Wbytes => 108,
            #[cfg(target_os = "linux")]
            ProcessField::Cnclwb => 109,
            #[cfg(target_os = "linux")]
            ProcessField::IOReadRate => 110,
            #[cfg(target_os = "linux")]
            ProcessField::IOWriteRate => 111,
            #[cfg(target_os = "linux")]
            ProcessField::IORate => 112,
            #[cfg(target_os = "linux")]
            ProcessField::CGroup => 113,
            #[cfg(target_os = "linux")]
            ProcessField::Oom => 114,
            #[cfg(target_os = "linux")]
            ProcessField::IOPriority => 115,
            #[cfg(target_os = "linux")]
            ProcessField::PercentCpuDelay => 116,
            #[cfg(target_os = "linux")]
            ProcessField::PercentIODelay => 117,
            #[cfg(target_os = "linux")]
            ProcessField::PercentSwapDelay => 118,
            #[cfg(target_os = "linux")]
            ProcessField::MPss => 119,
            #[cfg(target_os = "linux")]
            ProcessField::MSwap => 120,
            #[cfg(target_os = "linux")]
            ProcessField::MPsswp => 121,
            #[cfg(target_os = "linux")]
            ProcessField::Ctxt => 122,
            #[cfg(target_os = "linux")]
            ProcessField::SecAttr => 123,
            #[cfg(target_os = "linux")]
            ProcessField::AutogroupId => 127,
            #[cfg(target_os = "linux")]
            ProcessField::AutogroupNice => 128,
            #[cfg(target_os = "linux")]
            ProcessField::CCGroup => 129,
            #[cfg(target_os = "linux")]
            ProcessField::Container => 130,
            #[cfg(target_os = "linux")]
            ProcessField::MPriv => 131,
            #[cfg(target_os = "linux")]
            ProcessField::GpuTime => 132,
            #[cfg(target_os = "linux")]
            ProcessField::GpuPercent => 133,
            #[cfg(target_os = "linux")]
            ProcessField::IsContainer => 134,

            // macOS-specific fields
            #[cfg(target_os = "macos")]
            ProcessField::Translated => 100,
        }
    }
}
...
pub struct Process {
    ...
    pub last_deleted_lib_check_ms: u64, // Timestamp of last check (for throttling)
    ...
    pub updated: bool,
    pub was_shown: bool,
    pub seen_stamp_ms: u64, // monotonic timestamp when process was first seen
    pub tomb_stamp_ms: u64, // monotonic timestamp when process will be removed (0 = not tombed)
...
impl Process {
    ...
    pub fn new(pid: i32) -> Self {
        Process {
            ...
            updated: false,
            was_shown: false,
            seen_stamp_ms: 0, // Will be set when process is added to Machine
            tomb_stamp_ms: 0, // 0 = not tombed
            tagged: false,
        }
    }

    /// Check if this process is "new" (recently appeared)
    /// A process is new if:
    /// 1. The initial scan/draw cycle is complete (initial_scan_done == true)
    /// 2. It has NOT been shown before (was_shown == false)
    /// 3. It was seen within highlight_delay_secs
    ///
    /// This prevents all processes from being highlighted as "new" on startup.
    pub fn is_new(
        &self,
        monotonic_ms: u64,
        highlight_delay_secs: i32,
        initial_scan_done: bool,
    ) -> bool {
        // Don't highlight anything as new until the initial scan/draw is complete
        if !initial_scan_done {
            return false;
        }
        // was_shown means the process has been displayed at least once
        // So if was_shown is true, this is not a "new" process anymore
        if self.was_shown {
            return false;
        }
        if monotonic_ms < self.seen_stamp_ms {
            return false;
        }
        monotonic_ms - self.seen_stamp_ms <= 1000 * highlight_delay_secs as u64
    }

    /// Check if this process is a "tomb" (about to be removed)
    /// A tombed process has a non-zero tomb_stamp_ms
    ...
    pub fn make_command_str(&mut self, params: &CommandStrParams, tree_separator: &str) {
        ...
        let cmdline_basename_len = self
            .cmdline_basename_end
            .saturating_sub(self.cmdline_basename_start);
        ...
        let add_highlight = |highlights: &mut Vec<CmdlineHighlight>,
                             str_pos: usize,
                             offset: usize,
                             length: usize,
                             attr: u32,
        ...
        let comm_len = if have_comm_in_exe {
            exe_basename_len
        } else {
            0
        };
...
impl ProcessList {
    ...
    /// monotonic_ms is used to set the seen_stamp_ms for new processes (for highlight_changes)
    pub fn add(&mut self, mut process: Process, monotonic_ms: u64) {
        let pid = process.pid;
        if self.by_pid.contains(&pid) {
            ...
            if let Some(p) = self.processes.iter_mut().find(|p| p.pid == pid) {
                // Preserve highlight tracking fields from existing process
                process.was_shown = p.was_shown;
                process.seen_stamp_ms = p.seen_stamp_ms;
                process.tomb_stamp_ms = p.tomb_stamp_ms;
                *p = process;
            }
        } else {
            // New process - set seen_stamp_ms for highlight_changes feature
            process.seen_stamp_ms = monotonic_ms;
    ...
    ///
    /// If highlight_changes is enabled, processes that were shown but are no longer
    /// updated will be "tombed" (shown in red) for highlight_delay_secs before removal.
    pub fn cleanup(
        &mut self,
        cmd_params: Option<&CommandStrParams>,
        tree_separator: &str,
        ...
        self.processes.retain_mut(|p| {
            if p.tomb_stamp_ms > 0 {
                // Process is tombed - check if it's time to remove
                if monotonic_ms >= p.tomb_stamp_ms {
                    return false; // Remove
                }
                // Keep tombed process for now
                true
            } else if !p.updated {
                // Process no longer exists
                if highlight_changes && p.was_shown {
                    // Mark as tombed - will be removed after delay
                    p.tomb_stamp_ms = monotonic_ms + 1000 * highlight_delay_secs as u64;
                    true // Keep for now
                } else {
                    // Immediately remove
                    false
...
mod tests {
    ...
    fn test_process_field_base_title_falls_back_to_title() {
        ...
        assert_eq!(
            ProcessField::Command.base_title(),
            ProcessField::Command.title()
        );
        assert_eq!(
            ProcessField::State.base_title(),
            ProcessField::State.title()
        );
    ...
    fn test_process_field_from_name_common_fields() {
        ...
        assert_eq!(
            ProcessField::from_name("COMMAND"),
    ...
    fn test_process_field_from_name_aliases() {
        ...
        assert_eq!(
            ProcessField::from_name("RES"),
            Some(ProcessField::MResident)
        );
        assert_eq!(
            ProcessField::from_name("CPU%"),
            Some(ProcessField::PercentCpu)
        );
        assert_eq!(
            ProcessField::from_name("MEM%"),
            Some(ProcessField::PercentMem)
        );
        assert_eq!(ProcessField::from_name("TIME+"), Some(ProcessField::Time));
        ...
        assert_eq!(
            ProcessField::from_name("START"),
            Some(ProcessField::Starttime)
        );
        assert_eq!(
            ProcessField::from_name("CPU"),
            Some(ProcessField::Processor)
        );
        assert_eq!(
            ProcessField::from_name("SCHED"),
            Some(ProcessField::SchedulerPolicy)
        );
    }
    ...
    fn test_process_field_from_name_linux_fields() {
        assert_eq!(
            ProcessField::from_name("M_SHARE"),
            Some(ProcessField::MShare)
        );
        assert_eq!(ProcessField::from_name("SHR"), Some(ProcessField::MShare));
        assert_eq!(
            ProcessField::from_name("CGROUP"),
            Some(ProcessField::CGroup)
        );
        assert_eq!(ProcessField::from_name("OOM"), Some(ProcessField::Oom));
        assert_eq!(
            ProcessField::from_name("IO_PRIORITY"),
            Some(ProcessField::IOPriority)
        );
        assert_eq!(
            ProcessField::from_name("IO"),
            Some(ProcessField::IOPriority)
        );
        assert_eq!(
            ProcessField::from_name("IO_RATE"),
            Some(ProcessField::IORate)
        );
    }
    ...
    fn test_process_field_from_name_macos_fields() {
        assert_eq!(
            ProcessField::from_name("TRANSLATED"),
            Some(ProcessField::Translated)
        );
