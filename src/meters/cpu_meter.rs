...
use super::{
    draw_cpu_stacked_graph, draw_graph, CpuStackedGraphData, GraphData, Meter, MeterMode,
    CPU_STACKED_GRAPH_SEGMENTS, DEFAULT_GRAPH_HEIGHT,
};
...
pub struct CpuMeter {
    ...
    /// For single CPU/Average modes, only index 0 is used.
    /// For All/Left/Right modes, one entry per CPU in the range.
    graph_data: RefCell<Vec<GraphData>>,
    /// Stacked graph data for historical display with per-component colors
    /// For single CPU/Average modes, only index 0 is used.
    /// For All/Left/Right modes, one entry per CPU in the range.
    stacked_graph_data: RefCell<Vec<CpuStackedGraphData>>,
}
...
impl CpuMeter {
    pub fn new(cpu: Option<usize>) -> Self {
        CpuMeter {
            ...
            frequency: 0.0,
            cpu_count: 1,
            graph_data: RefCell::new(vec![GraphData::new()]),
            stacked_graph_data: RefCell::new(vec![CpuStackedGraphData::new()]),
        }
    }
    ...
    fn draw_cpu_bar_internal(
        ...
        if bar_width < 4 {
            crt.with_window(|win| {
                ...
                let _ = win.addstr(&format!("{:>3}", caption));
            });
            return;
        }
        ...
        // Calculate how many chars each segment takes and pre-compute color attrs
        ...
        for (value, color) in values {
            ...
            // Pre-compute color attr here, BEFORE entering with_window
            let attr = crt.color(*color);
            bar_chars.push((chars, attr));
            total_bar += chars;
        }
        ...
        let segment_bar_chars: Vec<char> = (0..bar_chars.len()).map(|i| crt.bar_char(i)).collect();
    ...
    fn draw_cpu_text_internal(
        ...
        // Pre-calculate all the color attributes we'll need
        let cpu_normal_attr = crt.color(ColorElement::CpuNormal);
        let cpu_system_attr = crt.color(ColorElement::CpuSystem);
        let cpu_nice_text_attr = crt.color(ColorElement::CpuNiceText);
        let cpu_irq_attr = crt.color(ColorElement::CpuIrq);
        let cpu_softirq_attr = crt.color(ColorElement::CpuSoftIrq);
        let cpu_steal_attr = crt.color(ColorElement::CpuSteal);
        let cpu_guest_attr = crt.color(ColorElement::CpuGuest);
        let cpu_iowait_attr = crt.color(ColorElement::CpuIOWait);

        // Build list of (string, attr) pairs to print
        let mut segments: Vec<(String, u32)> = Vec::new();

        segments.push((caption.to_string(), caption_attr));

        if detailed_cpu_time {
            // Detailed: show breakdown like C htop
            // Format: ":5.1% sy:5.1% ni:5.1% hi:5.1% si:5.1% st:5.1% gu:5.1% wa:5.1%"
            segments.push((":".to_string(), caption_attr));
            segments.push((format!("{:5.1}% ", user), cpu_normal_attr));
            segments.push(("sy:".to_string(), caption_attr));
            segments.push((format!("{:5.1}% ", system), cpu_system_attr));
            segments.push(("ni:".to_string(), caption_attr));
            segments.push((format!("{:5.1}% ", nice), cpu_nice_text_attr));
            segments.push(("hi:".to_string(), caption_attr));
            segments.push((format!("{:5.1}% ", irq), cpu_irq_attr));
            segments.push(("si:".to_string(), caption_attr));
            segments.push((format!("{:5.1}% ", softirq), cpu_softirq_attr));
            // st: only shown if steal is non-negative (i.e., supported)
            if steal >= 0.0 {
                segments.push(("st:".to_string(), caption_attr));
                segments.push((format!("{:5.1}% ", steal), cpu_steal_attr));
            }
            // gu: only shown if guest is non-negative (i.e., supported)
            if guest >= 0.0 {
                segments.push(("gu:".to_string(), caption_attr));
                segments.push((format!("{:5.1}% ", guest), cpu_guest_attr));
            }
            segments.push(("wa:".to_string(), caption_attr));
            segments.push((format!("{:5.1}% ", iowait), cpu_iowait_attr));
        } else {
            ...
            segments.push((":".to_string(), caption_attr));
            segments.push((format!("{:5.1}% ", user), cpu_normal_attr));
            segments.push(("sys:".to_string(), caption_attr));
            segments.push((format!("{:5.1}% ", kernel), cpu_system_attr));
            segments.push(("low:".to_string(), caption_attr));
            segments.push((format!("{:5.1}% ", nice), cpu_nice_text_attr));
            // vir: only shown if IRQ is non-negative (used as proxy for virtualization support)
            if irq >= 0.0 {
                segments.push(("vir:".to_string(), caption_attr));
                segments.push((format!("{:5.1}% ", virt), cpu_guest_attr));
            }
        }

        if show_cpu_frequency {
            segments.push(("freq: ".to_string(), caption_attr));
            segments.push((
        ...
        crt.with_window(|win| {
            ...
            for (s, attr) in &segments {
                ...
                let _ = win.mv(y, cur_x);
                let _ = win.addstr(&to_print);
                cur_x += printed_len;
            }
...
impl Meter for CpuMeter {
    ...
    fn height(&self) -> i32 {
        ...
        // Graph, StackedGraph and LED modes are multi-row per CPU meter
        match self.mode {
            MeterMode::Graph | MeterMode::StackedGraph => nrows * DEFAULT_GRAPH_HEIGHT,
            MeterMode::Led => nrows * 3,
            _ => nrows,
        }
    }
    ...
    fn draw(
        ...
        match self.mode {
            ...
            MeterMode::Graph => {
                match self.selection {
                    CpuSelection::All | CpuSelection::Left | CpuSelection::Right => {
                        // Multi-CPU graph mode: draw each CPU's graph in a grid layout
                        let (start_cpu, end_cpu) = self.cpu_range(machine.cpus.len());
                        let cpu_indices: Vec<usize> = (start_cpu..end_cpu).collect();
                        let num_cpus = cpu_indices.len();
                        ...
                        // Ensure we have enough graph data entries
                        {
                            let mut graph_data = self.graph_data.borrow_mut();
                            while graph_data.len() < num_cpus {
                                graph_data.push(GraphData::new());
                            }
                        }
                        ...
                        let col_width = width / ncol as i32;
                        let diff = (width % ncol as i32) as usize;
                        let graph_height = DEFAULT_GRAPH_HEIGHT;

                        for (i, &cpu_idx) in cpu_indices.iter().enumerate() {
                            if let Some(cpu) = machine.cpus.get(cpu_idx) {
                                // Calculate total for this CPU
                                let total = cpu.user_percent
                                    + cpu.nice_percent
                                    + cpu.system_percent
                                    + cpu.irq_percent
                                    + cpu.softirq_percent
                                    + cpu.steal_percent
                                    + if settings.account_guest_in_cpu_meter
                                        || !settings.detailed_cpu_time
                                    {
                                        cpu.guest_percent
                                    } else {
                                ...
                                let d = if col > diff { diff } else { col };
                                let col_x = x + (col as i32 * col_width) + d as i32;
                                let row_y = y + (row as i32 * graph_height);
                    ...
                    _ => {
                        // Single CPU or average - draw one graph
                        let total = self.user
                            + self.nice
                            + self.system
                            + self.irq
                        ...
                        // Record the value in graph data
                        {
                            ...
                            graph_data[0].record(normalized, settings.delay * 100);
                        }
            ...
            MeterMode::StackedGraph => {
                // StackedGraph mode: per-CPU stacked graphs with colored segments
                // Segments: user, nice, system, irq, softirq, steal, guest, iowait
                use crate::ui::ColorElement;

                // Pre-compute segment colors
                let segment_colors: [u32; CPU_STACKED_GRAPH_SEGMENTS] = [
                    crt.color(ColorElement::CpuNormal),  // user
                    crt.color(ColorElement::CpuNice),    // nice
                    crt.color(ColorElement::CpuSystem),  // system
                    crt.color(ColorElement::CpuIrq),     // irq
                    crt.color(ColorElement::CpuSoftIrq), // softirq
                    crt.color(ColorElement::CpuSteal),   // steal
                    crt.color(ColorElement::CpuGuest),   // guest
                    crt.color(ColorElement::CpuIOWait),  // iowait
                ];

                match self.selection {
                    CpuSelection::All | CpuSelection::Left | CpuSelection::Right => {
                        // Multi-CPU stacked graph mode: draw each CPU's stacked graph in a grid layout
                        let (start_cpu, end_cpu) = self.cpu_range(machine.cpus.len());
                        let cpu_indices: Vec<usize> = (start_cpu..end_cpu).collect();
                        let num_cpus = cpu_indices.len();
                        ...
                        // Ensure we have enough stacked graph data entries
                        {
                            let mut stacked_graph_data = self.stacked_graph_data.borrow_mut();
                            while stacked_graph_data.len() < num_cpus {
                                stacked_graph_data.push(CpuStackedGraphData::new());
                            }
                        }
                        ...
                        for (i, &cpu_idx) in cpu_indices.iter().enumerate() {
                            if let Some(cpu) = machine.cpus.get(cpu_idx) {
                                // Record segment values for this CPU (as percentages)
                                let segments: [f64; CPU_STACKED_GRAPH_SEGMENTS] = [
                                    cpu.user_percent,
                                    cpu.nice_percent,
                                    cpu.system_percent,
                                    cpu.irq_percent,
                                    cpu.softirq_percent,
                                    cpu.steal_percent.max(0.0), // Hide negative (unsupported) values
                                    cpu.guest_percent.max(0.0), // Hide negative (unsupported) values
                                    cpu.iowait_percent,
                                ];

                                // Record value in this CPU's stacked graph data
                                {
                    ...
                    _ => {
                        // Single CPU or average - draw one stacked graph
                        let segments: [f64; CPU_STACKED_GRAPH_SEGMENTS] = [
                            self.user,
                            self.nice,
                            self.system,
                            self.irq,
                            self.softirq,
                            self.steal.max(0.0), // Hide negative (unsupported) values
                            self.guest.max(0.0), // Hide negative (unsupported) values
                            self.iowait,
                        ];
                        ...
                        {
                            let mut stacked_graph_data = self.stacked_graph_data.borrow_mut();
                            if stacked_graph_data.is_empty() {
                                stacked_graph_data.push(CpuStackedGraphData::new());
                            }
                            stacked_graph_data[0].record(segments, settings.delay * 100);
                        }
                        ...
                        let caption = match self.selection {
                            CpuSelection::Average => "Avg",
                            CpuSelection::Cpu(n) => {
                                // Can't return format! from match, so handle separately
                                let display_id = if settings.count_cpus_from_one {
                                    n + 1
                                } else {
                                    n
                                };
                                // Use a static string for consistent lifetime
                                return draw_cpu_stacked_graph(
                                    crt,
                                    x,
                                    y,
                                    width,
                                    DEFAULT_GRAPH_HEIGHT,
                                    &stacked_graph_data[0],
                                    &format!("{:>3}", display_id),
                                    &segment_colors,
                                );
                            }
    ...
    fn supported_modes(&self) -> u32 {
        // CPU meter supports all modes including StackedGraph
...
mod tests {
    ...
    #[test]
    fn test_cpu_meter_height_graph_mode_scales_with_cpu_count() {
        let mut meter = CpuMeter::all(1);
        meter.cpu_count = 16;
        meter.set_mode(MeterMode::Graph);
        // In graph mode, AllCPUs draws each CPU's graph separately
        // 16 CPUs in 1 column = 16 rows, each 4 tall = 64
        assert_eq!(meter.height(), 16 * DEFAULT_GRAPH_HEIGHT);

        // With 4 columns: 16 CPUs / 4 = 4 rows of graphs
        let mut meter2 = CpuMeter::all(4);
        meter2.cpu_count = 16;
        meter2.set_mode(MeterMode::Graph);
        assert_eq!(meter2.height(), 4 * DEFAULT_GRAPH_HEIGHT);
    }
